...
static int av201x_regmask(struct av201x_priv *priv, u8 reg, u8 setmask,
			  u8 clrmask)
{
	int ret;
	u8 b = 0;
...
static int av201x_set_params(struct dvb_frontend *fe)
{
	...
	dev_dbg(&priv->i2c->dev, "%s() delivery_system=%d frequency=%d symbol_rate=%d\n",
	...
	n = DIV_ROUND_CLOSEST((c->frequency / 1000) << 17,
			      priv->cfg->xtal_freq / 1000);
	buf[2] = (u8)(n >> 9);
	buf[3] = (u8)(n >> 1);
...
static  int AV201x_agc[] = {
	0,  82,   100,  116,  140,  162,  173,  187,  210,  223,  254,  255
};

static  int AV201x_level_dBm_10[] = {
	90, -50,  -263, -361, -463, -563, -661, -761, -861, -891, -904, -910
};
...
static int av201x_get_rf_strength(struct dvb_frontend *fe, u16 *st)
{
	...
	/*
	 * Linear approximation of rssi value in segment (rssi values will be in
	 * 0.1dBm unit: '-523' means -52.3 dBm)
	 */
	*st = 1000 + ((y[index - 1] +
			((if_agc - x[index - 1]) * slope + 500) / 1000)) / 10;

	c->strength.len = 1;
	c->strength.stat[0].scale = FE_SCALE_DECIBEL;
	c->strength.stat[0].svalue = ((y[index - 1] +
			((if_agc - x[index - 1]) * slope + 500) / 1000)) * 100;
