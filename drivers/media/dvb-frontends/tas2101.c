...
static int tas2101_regmask(struct tas2101_priv *priv, u8 reg, u8 setmask,
			   u8 clrmask)
{
	int ret;
	u8 b = 0;
...
static int tas2101_read_signal_strength(struct dvb_frontend *fe, u16 *strength)
{
				    ...
				    struct i2c_adapter *i2c)
{
	...
	priv->muxc = i2c_mux_alloc(i2c, &i2c->dev, 2, 0, I2C_MUX_LOCKED,
...
static int tas2101_set_frontend(struct dvb_frontend *fe)
{
	...
	case SYS_DVBS2:
		...
		break;
	default:
		dev_warn(&priv->i2c->dev, "%s() unsupported delivery system (%d)\n",
				...
				struct dtv_frontend_properties *c)
{
	...
	if (reg > 33) {
		dev_dbg(&priv->i2c->dev, "%s() Unable to get current delivery system and mode.\n",
			__func__);
		reg = 0;
	}
