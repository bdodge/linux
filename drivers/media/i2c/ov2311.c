...
#define OV2311_LINK_FREQ		321500000
...
#define OV2311_REG_EXPOSURE		0x3500
 ...
 * border pixels.
 */
#define OV2311_NATIVE_WIDTH		1600U
#define OV2311_NATIVE_HEIGHT		1300U
#define OV2311_PIXEL_ARRAY_LEFT		0U
#define OV2311_PIXEL_ARRAY_TOP		0U
#define OV2311_PIXEL_ARRAY_WIDTH	16000U
 ...
 * mipi_datarate per lane 643Mbps
 */
...
static int ov2311_get_reso_dist(const struct ov2311_mode *mode,
...
static const struct ov2311_mode *
ov2311_find_best_fit(struct v4l2_subdev_format *fmt)
{
	...
	for (i = 0; i < ARRAY_SIZE(supported_modes); i++) {
		dist = ov2311_get_reso_dist(&supported_modes[i], framefmt);
...
__ov2311_get_pad_crop(struct ov2311 *ov2311, struct v4l2_subdev_pad_config *cfg,
		      unsigned int pad, enum v4l2_subdev_format_whence which)
{
	switch (which) {
	case V4L2_SUBDEV_FORMAT_TRY:
		return v4l2_subdev_get_try_crop(&ov2311->subdev, cfg, pad);
				...
				struct v4l2_subdev_selection *sel)
{
	...
	case V4L2_SEL_TGT_CROP: {
		...
		mutex_lock(&ov2311->mutex);
		sel->r = *__ov2311_get_pad_crop(ov2311, cfg, sel->pad,
						sel->which);
		mutex_unlock(&ov2311->mutex);
...
static int __ov2311_start_stream(struct ov2311 *ov2311)
{
	int ret;
...
static int __ov2311_stop_stream(struct ov2311 *ov2311)
{
	return ov2311_write_reg(ov2311->client, OV2311_REG_CTRL_MODE,
				OV2311_REG_VALUE_08BIT, OV2311_MODE_SW_STANDBY);
}

static int ov2311_s_stream(struct v4l2_subdev *sd, int on)
{
	struct ov2311 *ov2311 = to_ov2311(sd);
	...
	if (on) {
		...
		ret = __ov2311_start_stream(ov2311);
		if (ret) {
...
static int ov2311_s_power(struct v4l2_subdev *sd, int on)
{
	...
	if (on) {
		...
		ov2311->power_on = true;
	} else {
		pm_runtime_put(&client->dev);
		ov2311->power_on = false;
	}
...
static inline u32 ov2311_cal_delay(u32 cycles)
{
	return DIV_ROUND_UP(cycles, OV2311_XVCLK_FREQ / 1000 / 1000);
}

static int __ov2311_power_on(struct ov2311 *ov2311)
{
	int ret;
	u32 delay_us;
	...
	delay_us = ov2311_cal_delay(8192);
...
static void __ov2311_power_off(struct ov2311 *ov2311)
{
	if (!IS_ERR(ov2311->pwdn_gpio))
...
static int ov2311_runtime_resume(struct device *dev)
{
	...
	return __ov2311_power_on(ov2311);
}
...
static const struct dev_pm_ops ov2311_pm_ops = {
	SET_RUNTIME_PM_OPS(ov2311_runtime_suspend,
			   ov2311_runtime_resume, NULL)
};
...
static int ov2311_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_EXPOSURE:
		...
		ret = ov2311_write_reg(ov2311->client, OV2311_REG_EXPOSURE,
				       OV2311_REG_VALUE_24BIT, ctrl->val << 4);
		break;
...
static int ov2311_initialize_controls(struct ov2311 *ov2311)
{
	...
	ov2311->anal_gain = v4l2_ctrl_new_std(handler, &ov2311_ctrl_ops,
					      V4L2_CID_ANALOGUE_GAIN,
					      OV2311_GAIN_MIN, OV2311_GAIN_MAX,
					      OV2311_GAIN_STEP,
					      OV2311_GAIN_DEFAULT);
...
{
	return i2c_add_driver(&ov2311_i2c_driver);
}

static void __exit sensor_mod_exit(void)
{
	i2c_del_driver(&ov2311_i2c_driver);
}
