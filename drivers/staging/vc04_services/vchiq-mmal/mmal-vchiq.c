			     ...
			     struct mmal_msg *msg, u32 msg_len)
{
	...
	if (!mutex_trylock(&port->event_context_mutex)) {
		pr_err("dropping event 0x%x as context in use\n",
			     ...
			     struct mmal_buffer *buffer)
{
	...
	if (port->zero_copy && buffer->dma_buf && !buffer->vcsm_handle) {
		...
		ret = vc_sm_cma_import_dmabuf(buffer->dma_buf,
					      &buffer->vcsm_handle);
		if (ret) {
			pr_err("%s: vc_sm_cma_import_dmabuf_fd failed, ret %d\n",
			       __func__, ret);
			return ret;
		}
		...
		if (!buffer->vc_handle) {
			pr_err("%s: vc_sm_cma_int_handle failed %d\n",
			       __func__, ret);
...
static void free_all_event_contexts(struct vchiq_mmal_component *component)
{
	int idx;
...
MODULE_DESCRIPTION("VideoCore VCHIQ MMAL interface");
