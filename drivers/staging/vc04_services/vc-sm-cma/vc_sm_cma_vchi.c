	...
	while (1) {
		...
		if (!down_interruptible(&instance->io_sema)) {
			...
			while (!vchi_msg_peek
				...
				pr_err("%s: received msg id %08x", __func__,
				       reply->trans_id);
...
int vc_sm_cma_vchi_send_msg(struct sm_instance *handle,
			    enum vc_sm_msg_type msg_id, void *msg,
			    uint32_t msg_size, void *result,
			    uint32_t result_size, u32 *cur_trans_id,
			    uint8_t wait_reply)
{
	int status = 0;
...
{
	return vc_sm_cma_vchi_send_msg(handle, VC_SM_MSG_TYPE_ALLOC,
...
{
	return vc_sm_cma_vchi_send_msg(handle, VC_SM_MSG_TYPE_WALK_ALLOC,
				   0, 0, 0, 0, 0, 0);
}
...
{
	return vc_sm_cma_vchi_send_msg(handle, VC_SM_MSG_TYPE_ACTION_CLEAN,
...
int vc_sm_cma_vchi_client_vc_mem_req_reply(
				...
				uint32_t *cur_trans_id)
{
	return vc_sm_cma_vchi_send_msg(handle, VC_SM_MSG_TYPE_VC_MEM_REQUEST_REPLY,
				   msg, sizeof(*msg), 0, 0, cur_trans_id, 0);
}
