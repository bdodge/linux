...
struct sm_instance {
	...
	/* mutex protecting the lists below */
	struct mutex lock;
...
void vc_sm_response(struct sm_instance *instance, struct vc_sm_result_t *reply,
		    int reply_len)
{
	struct sm_cmd_rsp_blk *cmd = NULL;
	...
	if (&cmd->head == &instance->rsp_list) {
		...
		pr_debug("%s: reply too big (%u) %u, throw away...",
			 __func__, reply_len,
			 reply->trans_id);
	} else {
		...
		if (!down_interruptible(&instance->io_sema)) {
			...
			while (!vchi_msg_peek
				...
				if (reply->trans_id & 0x80000000)
					...
					vc_sm_cma_vpu_event(instance, reply,
							    reply_len);
				else
					vc_sm_response(instance, reply,
						       reply_len);
