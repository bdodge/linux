 ...
 *
 *
 * This driver has 1 main use of importing dmabufs from elsewhere for sharing
 * with the VPU.
 * The buffers to be imported MUST be contiguous as the VPU has no MMU.
 * In this case the native handle is a dmabuf. Releasing the resource inherently
 * comes from releasing the dmabuf, and this will trigger unmapping on the VPU.
 * The underlying allocation and our buffer structure are retained until the
 * VPU has confirmed that it has finished with it.
 *
 * Previous iterations of this driver have included the driver allocating memory
 * for sharing between userspace and the VPU. This is no longer supported as
 * the allocation can be done from a dma-heap and imported to vc-sm-cma.
 *
 * There is a second future use case of allocating buffers on behalf of the VPU.
 * This isn't used as yet, and could look to use in-kernel calls to a dma-heap
...
static void vc_sm_release_resource(struct vc_sm_buffer *buffer)
{
	...
	if (buffer->src_dma_buf)
		dma_buf_put(buffer->src_dma_buf);
	else
		pr_err("%s: Imported dmabuf already been put for buf %p\n",
		       __func__, buffer);
	buffer->src_dma_buf = NULL;
				...
				struct dma_buf_attachment *attachment)
{
	...
	return buf->src_dma_buf->ops->attach(buf->src_dma_buf,
						attachment);
}
				  ...
				  struct dma_buf_attachment *attachment)
{
	...
	buf->src_dma_buf->ops->detach(buf->src_dma_buf, attachment);
}
					  ...
					  enum dma_data_direction direction)
{
	...
	return buf->src_dma_buf->ops->map_dma_buf(attachment,
						     direction);
}
				...
				enum dma_data_direction direction)
{
	...
	buf->src_dma_buf->ops->unmap_dma_buf(attachment, table, direction);
}
...
int vc_sm_import_dmabuf_mmap(struct dma_buf *dmabuf, struct vm_area_struct *vma)
{
	...
	return buf->src_dma_buf->ops->mmap(buf->src_dma_buf, vma);
}
				...
				unsigned long offset)
{
	...
	return buf->src_dma_buf->ops->map(buf->src_dma_buf, offset);
}
				 ...
				 unsigned long offset, void *ptr)
{
	...
	buf->src_dma_buf->ops->unmap(buf->src_dma_buf, offset, ptr);
}
					  ...
					  enum dma_data_direction direction)
{
	...
	return buf->src_dma_buf->ops->begin_cpu_access(buf->src_dma_buf,
							    direction);
}
					...
					enum dma_data_direction direction)
{
	...
	return buf->src_dma_buf->ops->end_cpu_access(buf->src_dma_buf,
							  direction);
}
				 ...
				 struct dma_buf **imported_buf)
{
	...
	new_dma_buf = dma_buf_export(&exp_info);
	if (IS_ERR(new_dma_buf)) {
		ret = PTR_ERR(new_dma_buf);
		goto error;
	}

	vc_sm_add_resource(private, buffer);

	*imported_buf = new_dma_buf;
