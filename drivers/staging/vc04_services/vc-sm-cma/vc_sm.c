 ...
 * data.
 */
static void vc_sm_cma_add_resource(struct vc_sm_privdata_t *privdata,
 ...
 */
static void vc_sm_cma_release_resource(struct vc_sm_buffer *buffer, int force)
{
	mutex_lock(&sm_state->map_lock);
...
static
int vc_sm_cma_import_dma_buf_attach(struct dma_buf *dmabuf, struct device *dev,
...
static
void vc_sm_cma_import_dma_buf_detatch(struct dma_buf *dmabuf,
				      struct dma_buf_attachment *attachment)
{
	struct vc_sm_buffer *res = dmabuf->priv;
...
static
struct sg_table *vc_sm_cma_import_map_dma_buf(struct dma_buf_attachment *attach,
					      enum dma_data_direction dir)
{
	struct vc_sm_buffer *res = attach->dmabuf->priv;

	if (!res->import_dma_buf)
		return NULL;
	return res->import_dma_buf->ops->map_dma_buf(attach, dir);
}

static
void vc_sm_cma_import_unmap_dma_buf(struct dma_buf_attachment *attachment,
				    ...
				    enum dma_data_direction dir)
{
	...
	if (!res->import_dma_buf)
		return;
	res->import_dma_buf->ops->unmap_dma_buf(attachment, table, dir);
}

static
int vc_sm_cma_import_dmabuf_mmap(struct dma_buf *dmabuf,
...
static
void vc_sm_cma_import_dma_buf_release(struct dma_buf *dmabuf)
{
	struct vc_sm_buffer *res = dmabuf->priv;
	...
	vc_sm_cma_release_resource(res, 0);
}

static
void *vc_sm_cma_import_dma_buf_kmap(struct dma_buf *dmabuf,
...
static
void vc_sm_cma_import_dma_buf_kunmap(struct dma_buf *dmabuf,
...
static
int vc_sm_cma_import_dma_buf_begin_cpu_access(struct dma_buf *dmabuf,
					      enum dma_data_direction dir)
{
	struct vc_sm_buffer *res = dmabuf->priv;
...
static
int vc_sm_cma_import_dma_buf_end_cpu_access(struct dma_buf *dmabuf,
					    enum dma_data_direction dir)
{
	struct vc_sm_buffer *res = dmabuf->priv;
...
static const struct dma_buf_ops dma_buf_import_ops = {
	.map_dma_buf = vc_sm_cma_import_map_dma_buf,
	.unmap_dma_buf = vc_sm_cma_import_unmap_dma_buf,
	.mmap = vc_sm_cma_import_dmabuf_mmap,
	.release = vc_sm_cma_import_dma_buf_release,
	.attach = vc_sm_cma_import_dma_buf_attach,
	.detach = vc_sm_cma_import_dma_buf_detatch,
	.begin_cpu_access = vc_sm_cma_import_dma_buf_begin_cpu_access,
	.end_cpu_access = vc_sm_cma_import_dma_buf_end_cpu_access,
	.map_atomic = vc_sm_cma_import_dma_buf_kmap,
	.unmap_atomic = vc_sm_cma_import_dma_buf_kunmap,
	.map = vc_sm_cma_import_dma_buf_kmap,
	.unmap = vc_sm_cma_import_dma_buf_kunmap,
};
				 ...
				 struct dma_buf **imported_buf)
{
	...
	vc_sm_cma_add_resource(private, buffer);
...
void
vc_sm_cma_vpu_event(struct sm_instance *instance, struct vc_sm_result_t *reply,
		    int reply_len)
{
	...
	case VC_SM_MSG_TYPE_RELEASED:
	{
		...
		vc_sm_cma_release_resource(buffer, 0);
	}
	break;
	default:
...
/* Videocore connected.  */
static void vc_sm_cma_connected_init(void)
{
	int ret;
...
{
	pr_err("vc-sm-cma: Videocore CMA shared memory driver\n");
	...
	vchiq_add_connected_callback(vc_sm_cma_connected_init);
	return 0;
}
