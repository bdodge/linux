				      ...
				      struct vc5_gamma_entry *gamma)
{
	...
	/* Need to investigate why these registers don't always accept writes to
	 * them. Currently read-back and retry if incorrect.
	 */

	tries = 50;
	do {
		HVS_WRITE(offset, gamma->x_c_terms);
	} while (HVS_READ(offset) != gamma->x_c_terms && --tries);

	if (!tries && HVS_READ(offset) != gamma->x_c_terms)
		DRM_WARN("vc4: Failed to write gamma x_c_terms\n");

	tries = 50;
	do {
		HVS_WRITE(offset + 4, gamma->grad_term);
	} while (HVS_READ(offset + 4) != gamma->grad_term && --tries);

	if (!tries && HVS_READ(offset + 4) != gamma->grad_term)
		DRM_WARN("vc4: Failed to write gamma grad_term\n");
}
